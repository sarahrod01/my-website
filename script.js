...
const App = (() => {
  ...
  async function renderLibrary() {
    ...
    els.libList.innerHTML = `<div class="lib-grid" role="table" aria-label="Saved candidates">
      <div class="lib-row lib-headrow" role="row"><span>ID</span><span>Candidate</span><span class="c-role">Best suited to</span><span class="c-status">Status</span><span>Updated</span><span></span></div>${
      rows.map((c) => `<div class="lib-row${c.uid === state.candidate.uid ? ' is-current' : ''}" role="row" data-uid="${esc(c.uid)}">
        <span class="mono">${esc(c.id)}</span>
        <span class="c-name"><strong>${esc(CandidateModel.fullName(c) || '(no name)')}</strong><span class="sub">${esc(c.card.location)}</span></span>
        <span class="c-role">${esc(c.card.bestSuitedTo)}</span>
        <span class="c-status">${c.card.statusBadge ? `<span class="tag ${/recommend/i.test(c.card.statusBadge) ? 'tag-go' : 'tag-later'}">${esc(c.card.statusBadge)}</span>` : ''}</span>
        <span class="nowrap">${formatDate(c.updatedAt)}</span>
        <span class="actions-cell"><button type="button" class="btn btn-sm" data-lib="open">Open</button><button type="button" class="icon-btn danger" data-lib="delete" title="Delete" aria-label="Delete">${ICONS.remove}</button></span>
      </div>`).join('')}</div>`;
  }
  ...
  function warnOpenChecks() {
    const issues = CandidateModel.checks(state.candidate);
    if (issues.length) toast(`Note: ${issues.length} profile check${issues.length > 1 ? 's' : ''} still open (e.g. “${issues[0].msg}”)`, 'warn');
  }

  function printCv() {
    warnOpenChecks();
    try { window.print(); } // beforeprint/afterprint hide the [placeholders] while printing
    catch (e) { toast('Printing is blocked here. Open the file directly in Chrome or Edge.', 'error'); }
  }

  /* ---------- Download PDF (real .pdf file) ----------
     Uses two well-known open-source libraries, loaded only when needed:
     html2canvas (draws each A4 page) and jsPDF (writes the .pdf).
     Needs an internet connection the first time; falls back to Print. */
  const PDF_LIBS = [
  ...
  async function downloadPdf() {
    ...
    try {
      try {
        for (const [globalName, src] of PDF_LIBS) if (!window[globalName]) await loadScript(src);
      } catch (e) {
        toast('Download PDF needs an internet connection. Opening Print instead — choose “Save as PDF”.', 'error');
        setTimeout(printCv, 400);
        return;
      }
      warnOpenChecks();
      ...
      const pages = CvRenderer.renderPages(state.candidate, { showPrompts: false, measureEl: stage });
      const pdf = new window.jspdf.jsPDF({ unit: 'mm', format: 'a4', orientation: 'portrait', compress: true });
      for (let i = 0; i < pages.length; i++) {
        btn.textContent = `Page ${i + 1} of ${pages.length}…`;
        const canvas = await window.html2canvas(pages[i], { scale: 2.5, backgroundColor: '#ffffff', logging: false });
        if (i) pdf.addPage();
        pdf.addImage(canvas.toDataURL('image/jpeg', 0.92), 'JPEG', 0, 0, 210, 297, undefined, 'FAST');
      }
      const c = state.candidate;
      pdf.setProperties({ title: `${c.id} ${CandidateModel.displayName(c)} — Candidate Profile`.trim(), author: 'Nursiecare', subject: 'Candidate profile' });
      pdf.save(`${fileBase()}.pdf`);
      toast('PDF downloaded');
    } catch (e) {
      console.error(e);
      toast(`PDF failed (${e.message}). Use Print ▸ Save as PDF instead.`, 'error');
    } finally {
...
@media print{@page{size:A4;margin:0}body{background:none;padding:0;display:block}.cv-page{box-shadow:none;break-after:page}.cv-page:last-child{break-after:auto}}`;
  ...
  function bind() {
    ...
    els.libList.addEventListener('click', async (e) => {
      ...
      const row = e.target.closest('.lib-row[data-uid]');
